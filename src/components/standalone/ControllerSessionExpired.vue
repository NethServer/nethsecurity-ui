      ...
      <div class="mx-auto w-full max-w-md">
        <div class="bg-gray-50 px-6 py-12 shadow sm:rounded-lg sm:px-12 dark:bg-gray-900">
          <NeHeading tag="h4" class="mb-4">
            {{ t('login.controller_session_expired_title') }}
          </NeHeading>
          <div class="mb-6 text-sm text-gray-700 dark:text-gray-100">
            {{ t('login.controller_session_expired_description', { product: getProductName() }) }}
          </div>
          <NeInlineNotification
            kind="info"
            :title="t('login.controller_session_expired_hint_title')"
            :description="t('login.controller_session_expired_hint_description')"
            class="mb-6"
          />
