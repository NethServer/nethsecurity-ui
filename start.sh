...
podman run --rm --detach --network=host --volumes-from=$POD-vpn -v ./credentials.json:/credentials.json --pod $POD --name $POD-api -e API_DEBUG=1 -e FQDN=$(hostname -f) ghcr.io/nethserver/nextsec-api:latest
